...
describe('Tasks Integration Tests', () => {
    ...
    beforeAll(async () => {
        try {
            ...
            const { testInputDir: inputDir, testOutputDir: outputDir } =
            ...
            console.error('Setup failed:', {
                ...
                inputDirExists: testInputDir
                    ? fs.existsSync(testInputDir)
                    : false,
                outputDirExists: testOutputDir
                    ? fs.existsSync(testOutputDir)
                    : false,
            });
            throw error;
        }
    ...
    describe('POST /api/tasks', () => {
        it('should create a task with a local image', async () => {
            try {
                ...
                while (attempts2 < maxAttempts2) {
                    ...
                    console.log(
                        `Task status check (attempt ${attempts2 + 1}):`,
                        {
                            taskId,
                            status: finalStatus,
                    ...
                    if (finalStatus === TaskStatus.COMPLETED) {
                        ...
                        for (const image of taskResponse.body.images) {
                            ...
                            expect(fs.existsSync(processedImagePath)).toBe(
                                true,
                            );
                            const processedImageStats =
                                fs.statSync(processedImagePath);
                        ...
                        console.error('Task processing failed:', {
                            ...
                            inputStats: fs.existsSync(testImagePath)
                                ? fs.statSync(testImagePath)
                                : null,
                            outputDirContents: fs.existsSync(testOutputDir)
                                ? fs.readdirSync(testOutputDir)
                                : null,
                        });
                        throw new Error(
                            `Task processing failed: ${taskResponse.body.errorMessage}`,
                        );
                    }
                ...
                console.error('Test failure:', {
                    error: error.message,
                    stack: error.stack,
                    inputDirContents: fs.existsSync(testInputDir)
                        ? fs.readdirSync(testInputDir)
                        : null,
                    outputDirContents: fs.existsSync(testOutputDir)
                        ? fs.readdirSync(testOutputDir)
                        : null,
        ...
        it('should return 400 when no URL or local path is provided', async () => {
            ...
            expect(response.body.message).toBe(
                'URL or local image path needed.',
            );
        });
        ...
        it('should return 404 when local file is not found', async () => {
            ...
            expect(response.body.message).toBe(
                `${fileErrorResponseMessages.FILE_NOT_FOUND}: ${path.join(testInputDir, 'non-existent.jpg')}`,
            );
        });
    ...
    describe('GET /api/tasks/:id', () => {
        ...
        it('should return 404 when task is not found', async () => {
            ...
            expect(response.body.message).toBe(
                `${taskResponseErrorMessages.NOT_FOUND}: ${nonExistentId}`,
            );
        });
    });
        ...
        it('should complete the full task processing flow', async () => {
            ...
            try {
                ...
                while (attempts < maxAttempts) {
                    ...
                    console.log(
                        `Task status check (attempt ${attempts + 1}):`,
                        {
                            taskId,
                            status: finalStatus,
                ...
                console.error('Test failure:', {
                    ...
                    testImageStats: fs.existsSync(sampleImagePath)
                        ? fs.statSync(sampleImagePath)
                        : null,
                    testDirContents: fs.existsSync(testInputDir)
                        ? fs.readdirSync(testInputDir)
                        : null,
                    outputDirContents: fs.existsSync(testOutputDir)
                        ? fs.readdirSync(testOutputDir)
        ...
        it('should handle invalid image processing', async () => {
            ...
            try {
                ...
                while (attempts < maxAttempts) {
                    ...
                    finalStatus = response.body.status;
                    console.log(
                        `Invalid image task status check (attempt ${attempts + 1}):`,
                        {
                            taskId,
                            status: finalStatus,
                ...
                console.error('Invalid image test failure:', {
                    ...
                    invalidFileStats: fs.existsSync(invalidImagePath)
                        ? fs.statSync(invalidImagePath)
                        : null,
                    testDirContents: fs.existsSync(testInputDir)
                        ? fs.readdirSync(testInputDir)
                        : null,
